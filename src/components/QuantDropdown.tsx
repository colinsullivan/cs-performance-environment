...
import { SynkopaterSequencer, getGlobalQuant } from 'common/models/synkopater';
...
const QuantDropdown = (props: { sequencerId : string }) => {
  ...
  const seq : SynkopaterSequencer = useSelector((state) => getSequencer(state, props));
  ...
  return (
    <LabeledDropdown label="quant" options={options} value={globalQuant} onChange={handleChanged}/>
  );
};
